...
(function($) {
    $.imageArea = function(parent, id) {
        var options = parent.options,
            ...
            updateSelection = function() {
                ...
                $outline.css({
                    cursor: "default",
                    width: '20px', //area.width,
                    height: '20px', //area.height,
                    left: area.x,
                    top: area.y,
                    "z-index": area.z
                });
                ...
                $selection.css({
                    ...
                    width: '30px', //(area.width - 2 > 0) ? (area.width - 2) : 0,
                    height: '30px', //(area.height - 2 > 0) ? (area.height - 2) : 0,
                    left: area.x + 1,
                    top: area.y + 1,
                    "z-index": area.z + 2
                });
            ...
            refresh = function(sender) {
                switch (sender) {
                    ...
                    default:
                        // console.log("REfresh by Default");
                        updateSelection();
    ...
    $.imageSelectAreas.prototype.init = function(object, customOptions) {
        ...
        this.$overlay = $("<div class=\"select-areas-overlay\" />")
            .css({
                ...
                position: "absolute",
                width: '100%', //this.$image.width(),
                height: '100%', //this.$image.height()
            })
    ...
    $.imageSelectAreas.prototype._refresh = function() {
        ...
        // console.log("Areas Length "+nbAreas);
        this.$overlay.css({
    ...
    $.imageSelectAreas.prototype.newArea = function(event) {
        ...
        // console.log("New Mark Added "+this._areas[id]);
        if (event) {
    ...
    $.imageSelectAreas.prototype.add = function(options) {
        ...
        if ($.isArray(options)) {
            $.each(options, function(key, val) {
                // console.log("True Loop "+JSON.stringify(options));
                that._add(val);
            });
        } else {
            // console.log("Else Loop "+JSON.stringify(options));
            this._add(options);
        }
        this._refresh();
    ...
    $.imageSelectAreas.prototype.lastElementToBeDeleted = function() {
        // var that = this;
        // console.log(JSON.stringify(Object.keys(this._areas).length))
        // this._eachArea(function(area, id) {
        //     that.remove(area.id);
        // });
        // cancelEvent(event);
        if (Object.keys(this._areas).length > 0) {
            this.areas[(areas.length - 1)].id.deleteSelection();
        }
...
! function(e) {
    if ("object" == typeof exports && "undefined" != typeof module) module.exports = e();
        ...
        var f; "undefined" != typeof window ? f = window : "undefined" != typeof global ? f = global : "undefined" != typeof self && (f = self), f.html2canvas = e() } }(function() {
...
function deleteDeleteIcon() {
    $("div").removeClass('select-areas-delete-area');
}
